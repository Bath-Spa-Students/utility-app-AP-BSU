...
def vending():
    ...
    d = {"item": "Evian", "rate": 5.30, "available stock": 3}
    e = {"item": "Fiji", "rate": 6, "available stock": 1}
    items = [a, b, c, d, e]
    money = 0  # money in machine
    print("\nWelcome to the Waterfalling Machine")  # first interface display upon powered on
...
# Upon successful transaction selected item will be deducted if reduced to 0 it will now be hidden to item selection
            ...
            money -= rate
            print(f"{selected_item['item']} purchased. Cash remaining: AED: {money}")
